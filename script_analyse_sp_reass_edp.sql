...
drop table PRIME_CEDEE_ANT2021_EDP;
create table reass_db.PRIME_CEDEE_ANT2021_EDP as
(
select
i.*
...
from 
(select *from PRIME_CEDEE_ANT2021 where codebran not in (4,81,82,83,89))i,
...
--select count(*) from PRIME_CEDEE_ANT2021;



delete from reass_db.PRIME_CEDEE_ANT2021_REF_EDP;
--drop table reass_db.PRIME_CEDEE_ANT2021_REF_EDP;
insert into reass_db.PRIME_CEDEE_ANT2021_REF_EDP

--create table reass_db.PRIME_CEDEE_ANT2021_REF_EDP as
(
select  codeinte , 
        numepoli , 
      --  numeaven ,
...
from reass_db.PRIME_CEDEE_ANT2021_EDP where codeinte  not in (9999,9998,9995) 
group by codeinte , 
         numepoli , 
      --   numeaven ,
         DATEECHE ,
        CODEBRAN,codtyptr,desitrai,CODE_BRAN_RE , Tranche_Retention,
        libebran ,
        --liberisq , 
        coderisq 
...
drop table PRIME_CEDEES_ANT2021_acq_EDP;

CREATE table  reass_db.PRIME_CEDEES_ANT2021_acq_EDP as 

(
   select i.ANNEE, i.Annee_acquisition,
   j.codebran , j.codtyptr,j.desitrai,j.CODE_BRAN_RE,j.Tranche_Retention,
 --  i.BRANCHE1,
  -- i.BRANCHE2,
   i.Acquisition,
   i.NbrePolice,
...
(select
  ...
  -- numeaven ,
   ...
   reass_db.AnneePoliceAcquisition(trunc(DATEEFFE) ,trunc(DATEECHE) ,to_date('01/01/'||2009) ,to_date('31/12/'||2009)) Acquisition,
   EXPO,
   case when reass_db.AnneePoliceAcquisition(trunc(DATEEFFE) ,trunc(DATEECHE) ,to_date('01/01/'||2009) ,to_date('31/12/'||2009))>0 then 1 else 0 end kount,
   (case when reass_db.AnneePoliceAcquisition(trunc(DATEEFFE) ,trunc(DATEECHE) ,to_date('01/01/'||2009) ,to_date('31/12/'||2009))>0 then 1 else 0 end) *1 NbrePolice,
   2009 Annee_acquisition

from  reass_db.PRIME_CEDEE_ANT2021_REF_EDP)
...
(select
   datecomp,
   CODERISQ,
    codebran , codtyptr,Tranche_Retention,
    desitrai,
    CODE_BRAN_RE,
  -- COMMISSI,
  -- REGLCOMM,
  ...
  -- COMMGEST,
   case when reass_db.Exposition(trunc(DATEEFFE),trunc(DATEECHE))=0 then 0 else MONTCESS *reass_db.AnneePoliceAcquisition(trunc(DATEEFFE) ,trunc(DATEECHE) ,to_date('01/01/'||2009) ,to_date('31/12/'||2009))/reass_db.Exposition(trunc(DATEEFFE),trunc(DATEECHE)) end Prime_cess_acquise,
   case when reass_db.Exposition(trunc(DATEEFFE),trunc(DATEECHE))=0 then 0 else MONTPRIM *reass_db.AnneePoliceAcquisition(trunc(DATEEFFE) ,trunc(DATEECHE) ,to_date('01/01/'||2009) ,to_date('31/12/'||2009))/reass_db.Exposition(trunc(DATEEFFE),trunc(DATEECHE)) end Pnette_acquise,
   case when reass_db.Exposition(trunc(DATEEFFE),trunc(DATEECHE))=0 then 0 else MONTCOMM *reass_db.AnneePoliceAcquisition(trunc(DATEEFFE) ,trunc(DATEECHE) ,to_date('01/01/'||2009) ,to_date('31/12/'||2009))/reass_db.Exposition(trunc(DATEEFFE),trunc(DATEECHE)) end montcomm_acquise,
   case when reass_db.Exposition(trunc(DATEEFFE),trunc(DATEECHE))=0 then 0 else CESS_NT * reass_db.AnneePoliceAcquisition(trunc(DATEEFFE) ,trunc(DATEECHE) ,to_date('01/01/'||2009) ,to_date('31/12/'||2009))/reass_db.Exposition(trunc(DATEEFFE),trunc(DATEECHE)) end Cess_nt_acquise,
   case when reass_db.Exposition(trunc(DATEEFFE),trunc(DATEECHE))=0 then 0 else PRIMBRUT *reass_db.AnneePoliceAcquisition(trunc(DATEEFFE) ,trunc(DATEECHE) ,to_date('01/01/'||2009) ,to_date('31/12/'||2009))/(reass_db.Exposition(trunc(DATEEFFE),trunc(DATEECHE))) end Primbrut_acquise,
    2009 Annee_acquisition

from  REASS_DB.PRIME_CEDEE_ANT2021_EDP where codeinte not in (9999,9998,9995) )
group by to_char(datecomp,'YYYY'), Annee_acquisition, CODEBRAN,codtyptr,Tranche_Retention,desitrai,
CODE_BRAN_RE) j
...
delete from reass_db.PRIME_CEDEES_ANT2021_acq_EDP;

begin
FOR an IN (2009)..2025   loop

delete from reass_db.PRIME_CEDEES_ANT2021_acq_EDP where annee_acquisition=an;
insert into reass_db.PRIME_CEDEES_ANT2021_acq_EDP
(
   select i.ANNEE, i.Annee_acquisition,
   j.codebran , j.codtyptr,j.desitrai ,j.CODE_BRAN_RE,j.Tranche_Retention,
 --  i.BRANCHE1,
...
(select
   ...
   numepoli , 
  -- numeaven ,
   codebran , codtyptr, decode(codtyptr,'FQ','FQ',desitrai) desitrai,CODE_BRAN_RE,Tranche_Retention,
   --libebran ,
   --liberisq , 
   coderisq ,
   DATECOMP,
...
group by to_char(datecomp,'YYYY'), Annee_acquisition,
  ...
  -- numeaven ,
   --trunc(DATEECHE) ,
   codebran , codtyptr,desitrai,Tranche_Retention,CODE_BRAN_RE--,
   --libebran ,
   --liberisq , 
   --coderisq 
...
(select
   ...
   CODERISQ,
    ...
    CODE_BRAN_RE,Tranche_Retention,
  -- COMMISSI,
  -- REGLCOMM,
  -- COMMISS_CEDEE,
  -- COMISS_NETTE_DE_COASS,
...
from  REASS_DB.PRIME_CEDEE_ANT2021_EDP where codeinte not in (9999,9998,9995) )
group by to_char(datecomp,'YYYY'), Annee_acquisition, CODEBRAN,codtyptr,desitrai,Tranche_Retention,
CODE_BRAN_RE) j
where i.ANNEE=j.ANNEE(+) and i.Annee_acquisition=j.Annee_acquisition(+) and
   i.CODEBRAN=j.CODEBRAN(+)
   and  i.codtyptr=j.codtyptr(+)
...
---drop table PRIME_CEDEES_ANT2021_acq_f_edp;

delete from PRIME_CEDEES_ANT2021_acq_f_edp;
insert into PRIME_CEDEES_ANT2021_acq_f_edp

---create table PRIME_CEDEES_ANT2021_acq_f_edp as
(
...
from PRIME_CEDEES_ANT2021_acq_edp 
)
group by
ANNEE                 ,
ANNEE_ACQUISITION     ,
...
drop table TABLE_SINREASS_EDP;

create table  TABLE_SINREASS_EDP
...
---drop table sinistre_cedes_EDP;
delete from sinistre_cedes_EDP;
    insert into sinistre_cedes_EDP

---create table sinistre_cedes_EDP as
...
(
select annee,CODTYPTR,DESITRAI,codebran,CODE_BRAN_RE,Tranche_Retention,sum(sinpay) sinpay, sum(sinpay_ant) sinpay_ant, sum(recenc) recenc,
...
--)
  group by annee,CODTYPTR,codebran,DESITRAI,CODE_BRAN_RE,Tranche_Retention

)

union all
select * from reass_db.sinistre_cedes_reass_edp

;

---drop table tb_analyse_sp_reass_EDP;
delete from tb_analyse_sp_reass_EDP;
insert into tb_analyse_sp_reass_EDP

---create table tb_analyse_sp_reass_EDP as
(
select
 i.ANNEE,
 i.CODEBRAN      ,
...
select distinct to_char(ANNEE_ACQUISITION) ANNEE,nvl(CODEBRAN,0) CODEBRAN , nvl(CODTYPTR,'NON TRAITE PROPOR') CODTYPTR,
 nvl(DESITRAI,'NON TRAITE PROPOR') DESITRAI,nvl(tranche_retention,0) tranche_retention,nvl(CODE_BRAN_RE,1000) CODE_BRAN_RE from PRIME_CEDEES_ANT2021_acq_f_edp
union all
select distinct  ANNEE,nvl(CODEBRAN,0) CODEBRAN , nvl(CODTYPTR,'NON TRAITE PROPOR')  CODTYPTR,
 nvl(DESITRAI,'NON TRAITE PROPOR') DESITRAI,nvl(tranche_retention,0) 
 tranche_retention,nvl(CODE_BRAN_RE,1000) CODE_BRAN_RE from sinistre_cedes_edp where
 codebran not in (4,81,82,83,89)
...
from PRIME_CEDEES_ANT2021_acq_f_EDP ---where ANNEE_ACQUISITION>= 2014
group by
ANNEE_ACQUISITION ,
 nvl(CODEBRAN,0)       ,
 ...
 sinistre_cedes_EDP j,
 


(
select distinct
ANNEE,nvl(CODEBRAN,0) CODEBRAN,CODTYPTR,DESITRAI,CODE_BRAN_RE,tranche_retention,
count(distinct ANNEE||'_'||CODEINTE||'_'||NUMESINI) NBSIN 

from TABLE_SINREASS_edp
group by
ANNEE,nvl(CODEBRAN,0) ,CODTYPTR,DESITRAI,tranche_retention,CODE_BRAN_RE
)p,
...
count(distinct ANNEE||'_'||CODEINTE||'_'||NUMESINI) NBSIN_GRAVES 
 from
  TABLE_SINREASS_edp where graves=1


group by ANNEE,nvl(CODEBRAN,0),CODTYPTR,DESITRAI,tranche_retention,CODE_BRAN_RE
)q,
...
(
select distinct
ANNEE,nvl(CODEBRAN,0) CODEBRAN, 'XS' CODTYPTR,DESITRAN DESITRAI,CODE_BRAN_RE,nvl(tranche_retention,0) tranche_retention,
count(distinct ANNEE||'_'||CODEINTE||'_'||NUMESINI) NBSIN 

from TABLE_SINREASS_3_edp
group by
ANNEE,nvl(CODEBRAN,0) ,'XS',DESITRAN,nvl(tranche_retention,0) , CODE_BRAN_RE
)px,

(
select distinct
ANNEE,nvl(CODEBRAN,0) CODEBRAN, 'XS' CODTYPTR, DESITRAN DESITRAI,CODE_BRAN_RE,tranche_retention,
count(distinct ANNEE||'_'||CODEINTE||'_'||NUMESINI) NBSIN_GRAVES 
 from TABLE_SINREASS_3_edp where graves =1

group by ANNEE,nvl(CODEBRAN,0),'XS',DESITRAN,tranche_retention,CODE_BRAN_RE
)qx,
...
where i.ANNEE =    j.ANNEE (+)
 ...
 and  i.CODE_BRAN_RE  =a.CODE_BRAN_RE(+)
   and i.tranche_retention = nvl(a.tranche_retention(+),0)
 ...
 and  i.CODE_BRAN_RE  = nvl(p.CODE_BRAN_RE(+),1000)
  and i.tranche_retention = nvl(p.tranche_retention(+),0)
 ...
 and  i.CODE_BRAN_RE  = nvl(q.CODE_BRAN_RE(+),1000)
   and i.tranche_retention = nvl(q.tranche_retention(+),0)
 ...
 and  i.CODE_BRAN_RE  = nvl(px.CODE_BRAN_RE(+),1000)
  and i.tranche_retention = nvl(px.tranche_retention(+),0)
 ...
 and  i.CODE_BRAN_RE  = nvl(qx.CODE_BRAN_RE(+),1000)
   and i.tranche_retention = nvl(qx.tranche_retention(+),0)
